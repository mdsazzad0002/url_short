...
<script type="text/javascript">
     ...
     function notmatch() {
                 swal({
                title: "Sorry",
                text: "Please try again! varify code not match",
                icon: "warning",
                button: "ok",
                timer: 3000,
            });
         ...
         function notfound() {
                 swal({
                title: "Sorry",
                text: "We are not found data! signup now.",
                icon: "warning",
                button: "ok",
                timer: 4000,
...
if (!isset($_GET['gcode']) AND isset($_GET['checkpoint'])) {
	$checkpoint=$_GET['checkpoint'];
	if (mysqli_num_rows($con->query("SELECT * FROM `admin_user` WHERE `email`='$checkpoint'"))==1) {
// email send prepare
...
// if found gcode
    }else if (isset($_GET['gcode'])) {
    	          // $hidden='hidden';
     $checkpoint=$mail=$_GET['checkpoint'];
    if (isset($_GET['gcode'])) {
     $gcode=$_GET['gcode'];
     ...
     if (mysqli_num_rows($con->query("SELECT * FROM `admin_user` WHERE `email`='$mail' and `status`='$gcode'"))) {
         ...
         if ($succ) {
         	...
         	$password=md5($password);
         		$succ=$con->query("UPDATE `admin_user` SET `password`='$password' WHERE `email`='$mail'");
         		if ($succ) {
        ...
        <div class="box">
            ...
            <div class="content">
                ...
                <form action="">
                    ...
                    <input required <?php if (isset($_GET['checkpoint'])) {
                    	echo 'hidden';
                    };?> placeholder="Enter your Email Address"
                    type="text" value="<?php if(isset($checkpoint)){echo $checkpoint;}?>" name="checkpoint">
				<?php if (isset($_GET['checkpoint'])){?>
                   <input required  name="gcode" type="text" placeholder="Enter you Verification code">
                    <input required  type="text" name="password" placeholder="Enter your new password">
                <?php } ?>
